...
use Lara\Theme\Contracts\Theme as ThemeContract;
...
class Theme implements ThemeContract
{
    ...
    /**
     * @var \Lara\Theme\Repository
     */
    protected $config;
    /**
     * @var \Lara\Theme\Dispatcher
     */
    protected $events;
    /**
     * @var \Lara\Theme\Factory
     */
    protected $view;
    /**
     * @var \Lara\Theme\Asset
     */
    protected $asset;
    /**
     * @var \Lara\Theme\Filesystem
     */
    protected $files;
    /**
     * @var \Lara\Theme\Breadcrumb
     */
     ...
     * Theme constructor.
     *
     * @param \Illuminate\Config\Repository|\Lara\Theme\Repository     $config
     * @param \Illuminate\Events\Dispatcher|\Lara\Theme\Dispatcher     $events
     * @param \Illuminate\View\Factory|\Lara\Theme\Factory             $view
     * @param \Lara\Theme\Asset                                        $asset
     * @param \Illuminate\Filesystem\Filesystem|\Lara\Theme\Filesystem $files
     * @param \Lara\Theme\Breadcrumb                                   $breadcrumb
     */
                                ...
                                Breadcrumb $breadcrumb)
    {
        ...
        static::$namespace = is_null($namespace) ? static::$namespace : $namespace;
    ...
    {
        $signs = ['@'];
        $namespace = static::$namespace.((in_array(static::$namespace, $signs) || strlen(static::$namespace) ==0) ? '' : '.').$this->getThemeName();
    ...
    {
        // app('path.public') . '/' . $this->path($theme) . '/'
        $path = public_path($this->path($theme).'/');
        return is_dir($path);
    }
    ...
    public function all(){
        $themes = collect();
        $themeDir =  public_path($this->config->get('theme.themeDir'));

        if ($this->files->exists($themeDir)) {
            $scannedThemes = $this->files->directories($themeDir);
...
//        $this->asset->flush();
        ...
        if (!$this->view->exists($path)) {
            throw new \Exception("Layout [$this->layout] not found ($path)");
        }
...
//        }
    ...
    public function head(){
        $css = Assets::add('head_end')->css();
        $assets = Assets::reset()->add('head');
        return $assets->css().$css.Assets::js();
    }

    public function end(){
        return Assets::reset()->add('head_end')->js();
    }
